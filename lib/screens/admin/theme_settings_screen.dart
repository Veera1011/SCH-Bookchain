...
class _ThemeSettingsScreenState extends ConsumerState<ThemeSettingsScreen> {
    ...
    try {
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(content: Text('Theme updated successfully for all users!')),
        );
      }
    } catch (e) {
      if (mounted) {
  ...
  Future<void> _showAddDomainDialog(List<String> currentDomains) async {
    ...
    if (newDomain != null && newDomain.isNotEmpty && !currentDomains.contains(newDomain)) {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
        title: const Text('Global Theme Settings'),
        leading: IconButton(
      ...
      body: settingsAsync.when(
        data: (settings) {
          final currentColor = settings['primary_color'] as String? ?? '#1A3557';
          ...
          return SingleChildScrollView(
            ...
            child: Column(
              ...
              children: [
                    ...
                    child: Column(
                      ...
                      children: [
                        ...
                        Wrap(
                          ...
                          runSpacing: 8,
                          children: [
                            ...allowedDomains.map((domain) => Chip(
                              label: Text(domain),
                              backgroundColor: Theme.of(context).colorScheme.surfaceVariant.withOpacity(0.5),
                              deleteIcon: const Icon(Icons.close, size: 16),
                              onDeleted: _isLoading ? null : () async {
                                final newDomains = List<String>.from(allowedDomains)..remove(domain);
                                await _updateSettings('allowed_domains', newDomains);
                              },
                            )),
                            ActionChip(
                              ...
                              onPressed: _isLoading ? null : () => _showAddDomainDialog(allowedDomains),
                            ),
                          ],
                        ),
                      ],
                ...
                const SizedBox(height: 40),
                _buildSectionHeader('AI ASSISTANT', Icons.auto_awesome_outlined),
                const SizedBox(height: 16),
                Card(
                  elevation: 0,
                  shape: RoundedRectangleBorder(
                        ...
                        SizedBox(
                          ...
                          child: ElevatedButton(
                            onPressed: _isLoading
                                ? null
                                : () => _updateSettings('gemini_api_key', _aiKeyController.text.trim()),
                            ...
                            child: _isLoading 
                              ...
                              : const Text('SAVE AI CONFIGURATION', style: TextStyle(fontWeight: FontWeight.bold, letterSpacing: 1)),
                          ),
                        ),
                      ],
                    ),
